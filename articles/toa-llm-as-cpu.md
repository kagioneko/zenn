...
## 追記: Claude vs Codex でバックエンドを差し替えたら性格が真逆だった

同じTOAテープを `TOA_BACKEND=claude_cli` と `TOA_BACKEND=codex_cli` で流して比較した。

```bash
python -m toa.demo_compare --all-samples --backends claude_cli codex_cli
```

4サンプル全部の実行結果を並べると、明確なパターンが出た。

| パケット | Claude | Codex |
|---------|--------|-------|
| `s1x9`（XSSスキャン） | SQLiコードに対して「アクション不一致」を報告しつつ✓ | SQLiを検出して✓ |
| `s1f9`（XSS修正） | 不要なら✗、必要なら✓ | 常に✓（何かしら修正を適用） |
| `s1i9`（SQLiスキャン） | 前の命令で解決済みなら✗（スキップ） | 常に✓ |
| `v1c9`（最終検証・cleanコード） | ✓（クリーンを確認） | ✗（「unknown risk」で念のため失敗） |

**Claudeの挙動パターン**：「前の命令でもう解決したなら後続は不要」と自律判断して`success=false`を返す。命令の前提が実データと合わない場合も`false`を返す。後続のJIF分岐に影響させる。

**Codexの挙動パターン**：全命令を律義に実行する。クリーンなコードに修正命令を送っても「念のため適用」で`true`を返す。

同じ4文字の命令テープ、同じ実データ——なのに実行フローが変わる。バックエンドのLLMを差し替えるだけで、プログラムの分岐パターンが変化する。

これはTOAが「どのLLMを演算器として使うか」でプログラムの振る舞いが変わることを意味していて、逆に言えば **LLMの性格をTOAテープを通して定量的に測定できる**ということでもある。

Geminiが加わったら三者比較ができる（現在クォータ制限中、APIキーがあれば`TOA_BACKEND=gemini_api`で動く）。
